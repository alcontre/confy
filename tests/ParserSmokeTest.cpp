...
int main(int argc, char **argv)
{
   ...
   if (!Check(model.version == 9, "Expected version 9"))
      return 1;
   if (!Check(model.rootPath == "/tmp/confy-downloads", "Unexpected root path"))
      return 1;
   if (!Check(model.components.size() == 12, "Expected 12 components"))
      return 1;

   const auto &first = model.components[0];
   if (!Check(first.name == "my_name", "First component name mismatch"))
      return 1;
   ...
   if (!Check(first.artifact.enabled, "First artifact should be enabled"))
      return 1;
   if (!Check(first.source.branchOrTag == "master", "First source branch mismatch"))
      return 1;
   if (!Check(first.artifact.buildType == "Debug", "First artifact buildtype mismatch"))
      return 1;
   if (!Check(first.artifact.regexIncludes.size() == 2, "First artifact include regex count mismatch"))
      return 1;
   if (!Check(first.artifact.regexIncludes[0] == "\\.dll$", "First include regex[0] mismatch"))
      return 1;
   if (!Check(first.artifact.regexIncludes[1] == "^bin/", "First include regex[1] mismatch"))
      return 1;
   if (!Check(first.artifact.regexExcludes.size() == 1, "First artifact exclude regex count mismatch"))
      return 1;
   if (!Check(first.artifact.regexExcludes[0] == "/tests?/", "First exclude regex mismatch"))
      return 1;
   ...
   if (!Check(!second.source.shallow, "Second source should disable shallow clone via NoShallow"))
      return 1;
   if (!Check(!second.artifact.enabled, "Second artifact should be disabled"))
      return 1;
   ...
   if (!Check(last.source.enabled, "Last source should be enabled"))
      return 1;
   if (!Check(!last.artifact.enabled, "Last artifact should be disabled"))
      return 1;
